...
static void udp_msg_sent(char *server_ip, char *payload)
{
    ...
    int err = sendto(sock, payload, 720, 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
    if (err < 0) {
...
static void udp_client_task(void *pvParameters)
{
    ...
    uint8_t *gps_hr_data_1 = (uint8_t *) malloc(240);
    uint8_t *gps_hr_data_2 = (uint8_t *) malloc(240);
    uint8_t *gps_hr_data_3 = (uint8_t *) malloc(240);

    uint8_t *gps_hr_data_total = (uint8_t *) malloc(720);

    uint8_t *hr_data_to_mcu = (uint8_t *) malloc(4);
    memset(gps_hr_data_1, 0, 240);
    memset(gps_hr_data_2, 0, 240);
    memset(gps_hr_data_3, 0, 240);
    memset(gps_hr_data_total, 0, 720);
                ...
                case UART_DATA:
                    ...
                    else if (event.size == 22)
                    {
                        ...
                        if (data_integ_cnt == 10)
                        {
                            ...
                            if (data_240_cnt == 0)
                            {
                                printf("First 1 second ! \n");
                                for (int i=0; i<240; i++)
                                {
                                    gps_hr_data_1[i] = gps_hr_chunck_data[i];
                                }
                            }
                            else if (data_240_cnt  == 1)
                            {
                                printf("First 2 second ! \n");
                                for (int i=0; i<240; i++)
                                {
                                    gps_hr_data_2[i] = gps_hr_chunck_data[i];
                                }
                            }
                            ...
                            if (data_240_cnt >= 2)
                            {
                                ...
                                for (int i=0; i<240; i++)
                                {
                                    gps_hr_data_3[i] = gps_hr_chunck_data[i];
                                }

                                memcpy(gps_hr_data_total, gps_hr_data_1, 240);
                                memcpy(gps_hr_data_total + 240, gps_hr_data_2, 240);
                                memcpy(gps_hr_data_total + 240, gps_hr_data_3, 240);
                                ...
                                udp_msg_sent((const char *)udp_server_ip ,(const char *)gps_hr_data_total);

                                memset(gps_hr_data_1, 0, 240);
                                memcpy(gps_hr_data_1, gps_hr_data_2, 240);

                                memset(gps_hr_data_2, 0, 240);
                                memcpy(gps_hr_data_2, gps_hr_data_3, 240);
                                memset(gps_hr_data_3, 0, 240);
