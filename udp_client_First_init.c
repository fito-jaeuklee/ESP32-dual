...
static char *ble_hr_share_val = "0";
static int offset = 0;
...
//jeff: notify data.	
	...
	// heart rate value .	notify.value_len= 2, 00 48, 
        ...
        sprintf(ble_hr_share_val, "%x", p_data->notify.value[1]);
                ...
                case UART_DATA:
                    ...
                    else if (event.size == 24)
                    {
                        if (nvs_read_u8("is_written", nvs_handle) && reset_flag)
                        {
                            ...
                            // printf("check------------\n %s \n %s \n %s \n %s \n", udp_server_ip, wifi_ssid, remote_device_name);
                        ...
                        // memcpy((const char*)gps_hr_chunck_data, (const char*)dtmp);
                        // printf("-------dtmp data %d", strlen((const char*)dtmp));

                        // strcat((const char*)gps_hr_chunck_data, (const char*)ble_hr_share_val);
                        // strcat((const char*)gps_hr_chunck_data, "\n");
