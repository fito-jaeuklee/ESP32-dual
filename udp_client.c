...
uint16_t tx_payload_length = 588;
static int ble_device_connected  =0;
static uint16_t sec_tick_by_mcu = 0;

static int add_tx_packet_2hz(uint8_t *data_prt, int cnt)
{
...
// after 20sec, must move the existed data
	if(cnt > 10*2) // 
	{	
		// move the existed data
		// 	[28] = [0]
		//	[56] = [28]
		//	[84] = [56]
		//	.....
		...
		memmove(&tx_payload_buffer[560], data_prt, input_length);  //input new data
	}
	else
	{
		position = (input_length*cnt); // position : 28 56 84  ............... 560
		memmove(&tx_payload_buffer[position], data_prt, input_length); //input new data
	}
	return 1;
}


static int add_tx_packet_5hz(uint8_t *data_prt, int cnt)
{
	static uint16_t  position  =0;
	static uint8_t first_done = 0;
	int input_length = 65;
...
// after 10 sec, must move the existed data
	if(cnt > 10) 
	{	
		// move the existed data
		// 	[65] = [0]
		...
		memmove(&tx_payload_buffer[650], data_prt, input_length);  //input new data
	}
	else
	{
...
static int add_tx_packet_with_gps_coordinate_2hz(uint8_t *data_prt, int cnt)
{
	static uint16_t  position  =0;
	static uint8_t first_done = 0;
	int input_length = 48;
...
// after 5 sec, must move the existed data
	if(cnt > 14)
	{	
		// move the existed data
		// 	[48] = [0]
		...
		memmove(&tx_payload_buffer[672], data_prt, input_length); //input new data
	}
	else
	{
		position = (input_length*cnt); // position : 48 96 144 ..... 672 
		memmove(&tx_payload_buffer[position], data_prt, input_length); //input new data
	}
	return 1;
}
...
//jeff :  disconnect 		
    case ESP_GATTC_DISCONNECT_EVT:
        ble_connect = false;
        get_server = false;
	// yong_jun : scan time :20 minute 	
	 uint32_t duration = (20*60); 
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
        ...
        // ESP_LOGI(GATTC_TAG, "===== BLE Scan start ===========");
	// yong_jun : scan time :20 minute 	
	uint32_t duration = (20*60); 
        esp_ble_gap_start_scanning(duration);		
        break;
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
	    ...
	    adv_name = esp_ble_resolve_adv_data(scan_result->scan_rst.ble_adv,
			...
			//yong jun: check ble device connect ?? 
...
static void udp_client_task(void *pvParameters)
{
  ...
  // end	
   ...
   //yongjun: FW version 
     ESP_LOGE(TAG, "-------------------  ESP32 FW Version : 1.5.1 -------------------");
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (portTickType)portMAX_DELAY)) {
            ...
            switch (event.type) {
                ...
                case UART_DATA:
                    ...
                    //printf("uart event size = %d", event.size);
                    ...
                    else if (event.size == 12 ||event.size == 22)	// yong jun: sent 11 or 22 byte by mcu 
                    {
